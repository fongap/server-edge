...
if [[ -s "$contract" ]]; then
  jq -e '.schema_version == 1 and .producer == "proxy-hub" and .service == "subscription-feed" and .network == "edge_service_proxy_public" and .upstream == "http://proxy-feed:8080"' "$contract" >/dev/null \
    || die "invalid runtime proxy subscription contract"
  origin="$(jq -r '.public_origin // empty' "$contract")"
  if [[ -n "$origin" ]]; then
    [[ "$origin" =~ ^https://[A-Za-z0-9]([A-Za-z0-9.-]*[A-Za-z0-9])?$ ]] \
      || die "runtime subscription public_origin must be a canonical HTTPS origin"
